#!/usr/bin/env python3
"""Create a zoomed MoM-z14 composite using an already-mounted Google Drive."""
...
def main() -> None:
    if not Path("/content/drive/MyDrive").exists():
        raise RuntimeError("Google Drive is not currently mounted. Mount it once in Colab, then rerun this script.")
    ...
    for ax in (axes[0, 2], axes[1, 2]):
        ax.set_xticks([])
        ax.set_yticks([])
    fig.suptitle("MoM-z14 four-filter composite — no Drive remount", fontsize=18)
    ...
    print("DRIVE ACTION    Existing mount used; no mount or remount performed")
