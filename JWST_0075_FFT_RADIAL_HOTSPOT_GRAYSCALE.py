#!/usr/bin/env python3
"""
JWST_0075_FFT_RADIAL_HOTSPOT_GRAYSCALE.py

Create a synthetic black-and-white radial hotspot whose intensity falls
linearly from 256 at the center to 0 at a finite radius. Compute raw and
mean-subtracted 2-D FFT power spectra plus an azimuthally averaged FFT curve.
All displayed images use grayscale only.
"""
...
def make_dashboard(image: np.ndarray,
    ...
    grid = fig.add_gridspec(2, 3, height_ratios=[1.0, 1.0], wspace=0.28, hspace=0.34)
    ...
    ax_image.set_title("Radial grayscale hotspot\nwhite center, black exterior")
    ...
    raw_view = np.log10(raw_power / max(float(raw_power.max()), EPS) + EPS)
    centered_view = np.log10(centered_power / max(float(centered_power.max()), EPS) + EPS)
    ...
    fig.suptitle(
        "Black-and-white radial hotspot and its Fourier transform\n"
        "linear falloff from level 256 at the center to zero",
        fontsize=18,
        y=0.985,
    )

    output = PNG_DIR / f"{VERSION}_RADIAL_HOTSPOT_GRAYSCALE_FFT.png"
...
def main() -> None:
    ...
    print("COLOR MODE      grayscale only")
