# JWST_0054
# Simplified MoM-z14 redshift visualization.
# Each line-complex panel shows only the real spectrum, one observed peak,
# one rest wavelength, and z = observed/rest - 1.
...
def find_column(frame, startswith):
...
def draw_simple_spectrum(ax, row, wave, flux, color, small=False):
    import numpy as np
    ...
    if int(mask.sum()) >= 2:
        ...
        ax.plot(local_wave, local_flux, color=CYAN, linewidth=0.78, label="real JWST/MAST spectrum")
        ax.axvline(
...
def draw_z_summary(ax, lines, small=False):
    ...
    for index, row in enumerate(lines.itertuples()):
        ax.vlines(index, PUBLISHED_Z, row.z_pair, color=colors[index], linewidth=1.0, alpha=0.85)
        ax.text(
            index,
            row.z_pair,
            f" {row.z_pair:.4f}",
            ...
            ha="center",
            va="bottom" if row.z_pair >= PUBLISHED_Z else "top",
        )
    ...
    ax.set_xticklabels(
        ...
        color=TEXT,
        fontsize=6.6 if small else 7.8,
    )
    ax.set_ylabel("z from wavelength pair", fontsize=7.8 if small else 9.6)
    ax.set_title(
        "Five simple z calculations",
        fontsize=9.0 if small else 11.8,
        pad=8,
    )
    ...
    legend = ax.legend(
        loc="best",
        fontsize=6.2 if small else 7.5,
        facecolor="#07111f",
        edgecolor=GRID,
        framealpha=0.96,
    )
...
def create_individual_plots(lines, wave, flux):
    ...
    for index, row in enumerate(lines.itertuples()):
        ...
        draw_simple_spectrum(ax, row, wave, flux, POINT_COLORS[index], small=False)
        fig.suptitle(
            f"{VERSION} — {GALAXY}: one rest/observed wavelength pair",
            color=TEXT,
            fontsize=13.0,
            y=0.975,
        )
        path = PNG / f"{VERSION}_{GALAXY}_SIMPLE_PAIR_{int(row.n)}.png"
...
def create_dashboard(lines, wave, flux):
    ...
    axes = axes.ravel()
    for index, row in enumerate(lines.itertuples()):
        draw_simple_spectrum(axes[index], row, wave, flux, POINT_COLORS[index], small=True)
    mean_z = draw_z_summary(axes[5], lines, small=True)

    fig.suptitle(
        f"{VERSION} — {GALAXY}: only the wavelength pairs used to calculate z",
        color=TEXT,
        fontsize=15.5,
        fontweight="bold",
        y=0.984,
    )
    fig.subplots_adjust(left=0.055, right=0.985, top=0.947, bottom=0.055, hspace=0.36, wspace=0.18)
    path = PNG / f"{VERSION}_{GALAXY}_SIMPLE_Z_PAIR_DASHBOARD.png"
...
def create_summary_table(lines, line_csv, raw_csv, mean_z):
    import matplotlib.pyplot as plt
    ...
    table_rows.append(
        [
            "μ",
            "MEAN OF FIVE PAIR z VALUES",
            "",
            "",
            "",
            f"{mean_z:.6f}",
            f"{mean_z - PUBLISHED_Z:+.6f}",
            int(lines["sample_count"].sum()),
        ]
    )
    table_rows.append(
        [
            "★",
            "PUBLISHED JOINT-FIT REDSHIFT",
            "",
            "",
            f"{1.0 + PUBLISHED_Z:.6f}",
            f"{PUBLISHED_Z:.6f}",
            "REFERENCE",
            "",
        ]
    ...
    fig = plt.figure(figsize=(17.5, 6.8), facecolor=BG)
    ax = fig.add_axes([0.025, 0.12, 0.95, 0.76])
    ax.axis("off")
    ax.set_title(
        f"{VERSION} — {GALAXY}: simple rest/observed wavelength pairs",
        color=TEXT,
        fontsize=14.2,
        fontweight="bold",
        pad=14,
    ...
    table = ax.table(
        ...
        loc="center",
        cellLoc="center",
        colWidths=[0.045, 0.245, 0.115, 0.125, 0.11, 0.10, 0.125, 0.075],
    )
    ...
    fig.text(
        0.5,
        0.045,
        "Only two measured values enter each exploratory pair calculation: rest wavelength and observed peak wavelength.",
        color=MUTED,
        fontsize=9.0,
        ha="center",
    )

    table_path = PNG / f"{VERSION}_{GALAXY}_SIMPLE_Z_PAIR_TABLE.png"
    fig.savefig(table_path, dpi=245, facecolor=BG, edgecolor=BG)
    plt.show()
    plt.close(fig)

    result_csv = CSV / f"{VERSION}_{GALAXY}_SIMPLE_Z_PAIRS.csv"
...
def main():
    ...
    table_path, result_csv = create_summary_table(lines, line_csv, raw_csv, mean_z)
    ...
    print_table(
        [
            ...
            ("Line input CSV", str(line_csv)),
            ("Spectrum input CSV", str(raw_csv)),
            ...
            ("Plot rule", "one observed marker and one rest/observed pair per line complex"),
        ],
        ["Field", "Value"],
    )
