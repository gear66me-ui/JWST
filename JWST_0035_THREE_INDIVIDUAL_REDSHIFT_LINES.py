# JWST_0035
# Audit: Plot three individual redshift lines plus average and shared Gaussian fit. Python/matplotlib only. No AI images.

from pathlib import Path
from datetime import datetime, timezone
import os, sys, json, base64, subprocess, importlib, warnings

VERSION = "JWST_0035"
PROJECT = "THREE INDIVIDUAL REDSHIFT LINES"
REPO = "gear66me-ui/JWST"
BRANCH = "main"
DEST_ROOT = "OUTPUTS/JWST_0035_THREE_INDIVIDUAL_REDSHIFT_LINES"
...
def measure_triplet(wave, smooth, z_seed):
    ...
    for n, label, rest, species, weight in TRIPLET:
        ...
        rows.append({
            ...
            "z_line": z_line,
            "slope_m_equals_1_plus_z": 1 + z_line,
    ...
    mean_observed_peak_um = float(df["observed_peak_um"].mean())
    mean_slope = float((1 + df["z_line"]).mean())
    return df, z_mean, z_std, mean_observed_peak_um, mean_slope
...
def shared_gaussian_fit(wave, resid, z_seed):
    ...
    try:
        ...
        return {"z_fit": float(popt[0]), "slope_fit": 1 + float(popt[0]), "sigma_um": float(popt[1]), "x": x, "yfit": model(x, *popt), "status": "ok"}
    except Exception as exc:
...
def plot_three_individual_lines(df, z_mean, fit, mean_obs):
    import numpy as np
    import matplotlib.pyplot as plt
    colors = ["#38bdf8", "#fb7185", "#a78bfa"]
    xmin = df["rest_um"].min() - 0.0015
    xmax = df["rest_um"].max() + 0.0015
    xs = np.linspace(xmin, xmax, 200)
    individual_y = []
    for _, row in df.iterrows():
        individual_y.append((1 + row["z_line"]) * xs)
    individual_y = np.asarray(individual_y)

    fig, ax = plt.subplots(figsize=(16.8, 9.0))
    dark_axis(fig, ax)
    ax.fill_between(xs, individual_y.min(axis=0), individual_y.max(axis=0), color="#64748b", alpha=0.18, label="shaded band: span of the three individual line slopes")

    for i, (_, row) in enumerate(df.iterrows()):
        color = colors[i]
        m = 1 + row["z_line"]
        ax.plot(xs, m * xs, color=color, lw=1.25, alpha=0.92, label=f"{int(row['n'])}: individual slope m=1+z={m:.6f}")
        ax.scatter([row["rest_um"]], [row["observed_peak_um"]], s=110, color=color, edgecolor="#f8fafc", zorder=10)
        ax.text(row["rest_um"], row["observed_peak_um"], f"  {int(row['n'])} {row['line']}\nz={row['z_line']:.6f}", color="#f8fafc", fontsize=9, va="center", ha="left")

    avg_slope = 1 + z_mean
    ax.plot(xs, avg_slope * xs, color="#f97316", lw=2.25, label=f"orange: average z={z_mean:.6f}; slope={avg_slope:.6f}")
    if fit and "z_fit" in fit:
        ax.plot(xs, (1 + fit["z_fit"]) * xs, color="#22c55e", lw=1.75, ls="--", label=f"green dashed: shared Gaussian z={fit['z_fit']:.6f}; slope={1 + fit['z_fit']:.6f}")

    ax.axhline(mean_obs, color="#e2e8f0", lw=0.9, ls=":", alpha=0.70, label=f"dotted horizontal: numerical mean of observed peaks = {mean_obs:.6f} µm")
    ax.set_xlabel("Rest-frame wavelength, micron")
    ax.set_ylabel("Observed peak wavelength, micron")
    ax.set_title(f"{VERSION} — three individual redshift lines, average line, and Gaussian shared-z line")
    yvals = list(df["observed_peak_um"]) + list((avg_slope * xs)) + [mean_obs]
    if fit and "z_fit" in fit:
        yvals += list((1 + fit["z_fit"]) * xs)
    pad = (max(yvals) - min(yvals)) * 0.22
    ax.set_ylim(min(yvals) - pad, max(yvals) + pad)
    ax.set_xlim(xmin, xmax)
    legend(ax, "upper left")
    fig.tight_layout()
    p = PNG / f"{VERSION}_THREE_INDIVIDUAL_LINES_PLUS_AVERAGE.png"
...
def plot_triplet_zoom(wave, flux, cont, resid, smooth, df, fit, z_mean, unit):
    ...
    if fit and "x" in fit:
        ax2.plot(fit["x"], fit["yfit"], color="#22c55e", lw=1.60, label=f"Gaussian shared-z fit: z={fit['z_fit']:.6f}")
    for i, (_, row) in enumerate(df.iterrows()):
        color = colors[i]
        x = row["observed_peak_um"]
        for ax in [ax1, ax2]:
            ax.axvline(x, color=color, lw=1.0, alpha=0.80)
            ax.text(x, ax.get_ylim()[1], f" {int(row['n'])}", color="#050712", fontsize=11, weight="bold", ha="left", va="top", bbox=dict(boxstyle="circle,pad=0.24", facecolor=color, edgecolor="#f8fafc"))
        ax2.scatter([x], [row["peak_residual_flux"]], s=70, color=color, edgecolor="#f8fafc", zorder=10)
    ax1.set_title(f"{VERSION} — spectrum peaks 1, 2, 3 used for three individual redshift slopes")
    ...
    lane.text(0.02, 0.985, "Three measured peaks", color="#f8fafc", fontsize=16, weight="bold", va="top")
    lane.text(0.02, 0.940, "Each measured peak creates one slope: m_i = λobs_i / λrest_i = 1 + z_i", color="#cbd5e1", fontsize=9.5, wrap=True, va="top")
    y = 0.835
    for i, (_, row) in enumerate(df.iterrows()):
        color = colors[i]
        lane.text(0.02, y, f"{int(row['n'])}", color="#050712", fontsize=11, weight="bold", bbox=dict(boxstyle="circle,pad=0.30", facecolor=color, edgecolor="#f8fafc"))
        lane.text(0.13, y + 0.030, str(row["line"]), color=color, fontsize=12, weight="bold")
        lane.text(0.13, y - 0.006, f"λrest = {row['rest_um']:.6f} µm", color="#94a3b8", fontsize=8.6)
        lane.text(0.13, y - 0.040, f"λobs = {row['observed_peak_um']:.6f} µm", color="#dbeafe", fontsize=8.6)
        lane.text(0.13, y - 0.074, f"z = {row['z_line']:.6f}; m = {1 + row['z_line']:.6f}", color="#f8fafc", fontsize=8.9)
        y -= 0.205
    lane.text(0.02, 0.090, f"Orange line is not one observed wavelength.\nIt is the average slope:\nmean m = 1 + mean(z_i) = {1 + z_mean:.6f}", color="#f8fafc", fontsize=10, bbox=dict(boxstyle="round,pad=0.48", facecolor="#020617", edgecolor="#475569"))
    fig.tight_layout()
    p = PNG / f"{VERSION}_TRIPLET_PEAKS_1_2_3.png"
    fig.savefig(p, dpi=280, facecolor=fig.get_facecolor())
    plt.show()
    return p


def plot_table(df, z_mean, z_std, mean_obs, mean_slope, fit):
    import matplotlib.pyplot as plt
    view = df[["n", "line", "rest_um", "observed_peak_um", "z_line", "slope_m_equals_1_plus_z"]].copy()
    for c in ["rest_um", "observed_peak_um", "z_line", "slope_m_equals_1_plus_z"]:
        view[c] = view[c].map(lambda x: f"{x:.6f}")
    view.columns = ["#", "line", "rest λ µm", "observed peak λ µm", "z_i", "slope m_i"]
    fig, ax = plt.subplots(figsize=(16.8, 5.2))
    fig.patch.set_facecolor("#050712")
    ax.set_facecolor("#050712")
    ax.axis("off")
    fit_txt = f" | Gaussian slope={1 + fit['z_fit']:.6f}" if fit and "z_fit" in fit else ""
    ax.set_title(f"{VERSION} — three individual slopes | mean z={z_mean:.6f}, mean slope={mean_slope:.6f}, mean observed peak={mean_obs:.6f} µm{fit_txt}", color="#f8fafc", fontsize=12.7, pad=12)
    ...
    fig.tight_layout()
    p = PNG / f"{VERSION}_THREE_SLOPES_TABLE.png"
...
def display_links(rows):
    ...
    try:
        ...
        if previews:
            display(HTML(f'<div style="background:#050712;color:#f8fafc;padding:16px;border-radius:10px;border:1px solid #475569"><h2>Three individual redshift lines</h2>{previews}</div>'))
    except Exception as exc:
...
def main():
    ...
    df, z_mean, z_std, mean_obs, mean_slope = measure_triplet(wave, smooth, z_seed)
    fit = shared_gaussian_fit(wave, resid, z_mean)

    import pandas as pd
    triplet_csv = CSV / f"{VERSION}_THREE_INDIVIDUAL_LINE_SLOPES.csv"
    summary_csv = CSV / f"{VERSION}_SLOPE_SUMMARY.csv"
    df.to_csv(triplet_csv, index=False)
    pd.DataFrame([{
        "z_seed_line_comb": z_seed,
        "mean_z_from_three_lines": z_mean,
        "sample_std_z": z_std,
        "mean_slope_from_three_lines": mean_slope,
        "mean_observed_peak_um": mean_obs,
        "gaussian_fit_z": fit.get("z_fit") if fit and "z_fit" in fit else None,
        "gaussian_fit_slope": fit.get("slope_fit") if fit and "slope_fit" in fit else None,
        "note": "orange average line uses mean slope = mean(lambda_obs/lambda_rest), not a single y value",
        "mast_product": PRODUCT,
        "download_status": status,
    }]).to_csv(summary_csv, index=False)

    png1 = plot_three_individual_lines(df, z_mean, fit, mean_obs)
    png2 = plot_triplet_zoom(wave, flux, cont, resid, smooth, df, fit, z_mean, unit)
    png3 = plot_table(df, z_mean, z_std, mean_obs, mean_slope, fit)
    ...
    print_table([
        ...
        ("Three individual curves", "one line per peak: y = (lambda_obs_i/lambda_rest_i) x"),
        ("Orange average line", "y = mean(lambda_obs_i/lambda_rest_i) x = (1 + mean z) x"),
        ("Green dashed line", "shared-z Gaussian fit"),
        ("Shaded band", "range between the three individual slopes"),
        ("Mean observed peak um", f"{mean_obs:.6f}"),
        ("Mean z", f"{z_mean:.6f}"),
        ("Mean slope", f"{mean_slope:.6f}"),
        ("Gaussian z", f"{fit.get('z_fit'):.6f}" if fit and "z_fit" in fit else "not available"),
    ], ["Field", "Value"])
    print("\nINDIVIDUAL LINE SLOPES")
    print_table([(int(r.n), r.line, f"{r.rest_um:.6f}", f"{r.observed_peak_um:.6f}", f"{r.z_line:.6f}", f"{r.slope_m_equals_1_plus_z:.6f}") for r in df.itertuples()], ["#", "Line", "Rest um", "Obs peak um", "z_i", "m_i"])
    print("\nOUTPUT HTTP LINKS")
    print_table([(r["type"], r["status"], r["file"], r["raw_url"] or "local only") for r in rows], ["Type", "Status", "File", "HTTP link"])
    display_links(rows)
    print("\nCOMMENTS")
    print("This version plots the three individual redshift lines separately, plus the orange average line and green dashed Gaussian line.")
    print("The dotted horizontal line is the numerical average of observed peak wavelengths; it is not the redshift slope.")
