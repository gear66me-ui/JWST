#!/usr/bin/env python3
"""
JWST_0056_FFT_TOY_GAUSSIAN_2ROW_NOCROSS.py

Two-row toy FFT learning dashboard:
1) pure Gaussian lamp,
2) high-pass Gaussian.

This revision removes ALL guide crosshairs from Column 3 (the 2-D FFT panels)
so the Fourier images display only the actual FFT power with no overplotted
center lines, glows, or markers.
"""
    ...
    return [
        {
            ...
            "image": base,
            "note": "Smooth hotspot: Fourier power is a compact smooth central peak.",
        },
        {
...
def radial_profile(power: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    ridx = np.floor(np.hypot(xx - cx, yy - cy)).astype(int)
    radius = np.arange(ridx.max() + 1, dtype=float)
    values = np.full(radius.shape, np.nan)
    for i in range(len(radius)):
        mask = ridx == i
        if np.any(mask):
            values[i] = np.mean(power[mask])
...
def make_dashboard(results: list[dict]) -> Path:
    ...
    for row, record in enumerate(results):
        ...
        ax.set_title("2-D FFT power — no guides", fontsize=11)
    ...
    fig.suptitle(
        "Toy Gaussian FFT demo — two-row no-cross revision\n"
        "Column 3 contains no overplotted guide lines, glows, or markers",
        fontsize=18,
    )
    output = PNG_DIR / f"{VERSION}_TOY_GAUSSIAN_2ROW_NOCROSS_DASHBOARD.png"
...
def make_radial_plot(results: list[dict]) -> Path:
    ...
    ax.grid(alpha=0.20)
    ax.legend()
    output = PNG_DIR / f"{VERSION}_TOY_GAUSSIAN_2ROW_NOCROSS_RADIAL.png"
    fig.savefig(output, dpi=220, bbox_inches="tight", facecolor=fig.get_facecolor())
    plt.show()
    plt.close(fig)
    return output
...
def make_summary(results: list[dict]) -> tuple[Path, Path]:
    ...
    for record in results:
        ...
        peak_frequency = float(frequency[mask][idx])
        rows.append({
            "case": record["title"],
    ...
    csv_path = CSV_DIR / f"{VERSION}_TOY_GAUSSIAN_2ROW_NOCROSS_SUMMARY.csv"
    ...
    shown.columns = ["Case", "Dominant f [cyc/pix]", "Equivalent period [pix]", "Center power", "Interpretation"]
    ...
    ax.set_title("Toy Gaussian FFT two-row no-cross summary", fontsize=15, pad=16)
    png_path = PNG_DIR / f"{VERSION}_TOY_GAUSSIAN_2ROW_NOCROSS_TABLE.png"
...
def main() -> None:
    ...
    print("Change       Removed all Column 3 guide crosshairs")
