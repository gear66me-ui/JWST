...
print("MOM-Z14 BLACK PAPER-STYLE SPECTRUM WITH ORANGE-RED REFERENCE LINES")
print("-"*112)
...
def draw(*_):
    ...
    with out:
        ...
        for label,xpos in LINES.items():
            if x0<=xpos<=x1:
                ax1.axvline(xpos,color=REFERENCE_COLOR,ls="--",lw=0.9,zorder=0)
                ax1.text(xpos,ymax_box.value*0.965,label,rotation=90,ha="right",va="top",fontsize=10,color=REFERENCE_COLOR)
        ...
        last["fig"]=fig; last["name"]=f"{VERSION}_MOM_Z14_{x0:.2f}_{x1:.2f}UM_{int(nsig)}SIGMA_BLACK_ORANGE_RED_LINES.png".replace(".","p")
...
default_png=PNG_DIR/f"{VERSION}_MOM_Z14_1p0_TO_3p0UM_1SIGMA_BLACK_ORANGE_RED_LINES.png"
