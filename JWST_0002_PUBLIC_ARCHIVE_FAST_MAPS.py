# JWST_0002
# Audit: fast JWST metadata mapping through MAST; GitHub/Colab remote-run friendly.
...
VERSION = "JWST_0002"
PROJECT = "JWST PUBLIC ARCHIVE FAST MAPPING"
...
# Smaller radii make the first live MAST pass much faster.
TARGETS = [
    {"target_group": "SMACS 0723 Deep Field", "ra_deg": 110.8375, "dec_deg": -73.4550, "radius_deg": 0.10},
    {"target_group": "Stephan's Quintet", "ra_deg": 339.0144, "dec_deg": 33.9750, "radius_deg": 0.12},
    {"target_group": "NGC 3324 Carina", "ra_deg": 159.3330, "dec_deg": -58.6250, "radius_deg": 0.12},
    {"target_group": "Southern Ring NGC 3132", "ra_deg": 151.7583, "dec_deg": -40.4364, "radius_deg": 0.10},
    {"target_group": "Pillars of Creation M16", "ra_deg": 274.7000, "dec_deg": -13.8067, "radius_deg": 0.12},
    {"target_group": "NGC 628 PHANGS", "ra_deg": 24.1740, "dec_deg": 15.7837, "radius_deg": 0.14},
    {"target_group": "Orion Bar", "ra_deg": 83.8390, "dec_deg": -5.4170, "radius_deg": 0.10},
]

KEEP_COLUMNS = [
    "obs_collection", "instrument_name", "dataproduct_type", "calib_level",
    "intentType", "target_name", "proposal_id", "proposal_pi", "obs_id",
    "obsid", "s_ra", "s_dec", "t_min", "t_max", "t_exptime",
    "filters", "em_min", "em_max", "dataRights"
]
...
def table_to_dataframe(table, target_group, query_ra, query_dec, query_radius):
    import pandas as pd
    ...
    df["query_ra_deg"] = query_ra
    df["query_dec_deg"] = query_dec
    df["query_radius_deg"] = query_radius
...
def query_jwst_metadata():
    ...
    for index, item in enumerate(TARGETS, start=1):
        print(f"QUERY {index:02d}/{len(TARGETS):02d} | {item['target_group']} | radius={item['radius_deg']:.3f} deg", flush=True)

        try:
            coord = SkyCoord(item["ra_deg"], item["dec_deg"], unit="deg", frame="icrs")

            # Faster than querying every archive first and filtering locally.
            obs = Observations.query_criteria(
                coordinates=coord,
                radius=item["radius_deg"] * u.deg,
                obs_collection="JWST",
                intentType="science",
            )
            ...
            failures.append((item["target_group"], str(exc)[:180]))
            print(f"  failed={str(exc)[:100]}", flush=True)
...
def save_tables(df):
    ...
    count_col = "obs_id" if "obs_id" in df.columns else "instrument_short"
    summary = (
        df.groupby(["target_group", "instrument_short"])
        .agg(observation_count=(count_col, "count"), exposure_seconds=("t_exptime", "sum"))
        .reset_index()
...
def save_plots(df):
    ...
    if len(plot_df) > 0:
        ...
        for name, g in plot_df.groupby("target_group"):
            ax.scatter(g["s_ra"], g["s_dec"], s=8, alpha=0.70, label=name)
        ax.invert_xaxis()
        ax.set_xlabel("Right Ascension, deg")
        ax.set_ylabel("Declination, deg")
        ax.set_title("JWST observation positions around selected target regions")
    ...
    if "gal_l_deg" in df.columns and "gal_b_deg" in df.columns:
        ...
        if len(gdf) > 0:
            ...
            for name, g in gdf.groupby("instrument_short"):
                ax.scatter(g["gal_l_deg"], g["gal_b_deg"], s=8, alpha=0.70, label=name)
    ...
    filters = df["filters_clean"].replace("", "UNSPECIFIED").value_counts().head(20).sort_values(ascending=True)
    if len(filters) > 0:
        fig, ax = plt.subplots(figsize=(10, 7))
        ax.barh(filters.index, filters.values)
    ...
    timeline = df.dropna(subset=["t_min"]).copy() if "t_min" in df.columns else df.iloc[0:0].copy()
    if len(timeline) > 0:
...
def main():
    ...
    print_table([
        ...
        ("Targets", len(TARGETS)),
        ("Mode", "Direct MAST JWST query, metadata only, no FITS download"),
    ], ["Field", "Value"])
    ...
    if df.empty:
        ...
        for target, g in df.groupby("target_group"):
            ...
            instruments = ", ".join(sorted(set(g["instrument_short"].astype(str))))[:46]
    ...
    print("")
    print("COMMENTS")
    print("This version prints progress after every target query.")
    print("It queries only JWST science metadata and avoids FITS/product downloads.")
    print("Reduced search radii make the first archive pass faster and easier to debug.")
