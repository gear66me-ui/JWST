...
print("MOM-Z14 BLACK-BACKGROUND PAPER-STYLE SPECTRUM WIDGET — 1.0 TO 3.0 MICRONS")
print("-" * 112)
    ...
    def pick(*opts):
        for key in opts:
            if key.upper() in names:
                return np.ravel(np.asarray(tab[names[key.upper()]], dtype=float))
        return None
...
if wave_all is None or flux_all is None:
    raise RuntimeError(f"Required SPEC1D columns unavailable: {tab.names}")

# Convert native f_nu [microJy] to paper f_lambda units.
c_ang_s = 2.99792458e18
lam_ang = wave_all * 1e4
conv = 1e-6 * 1e-23 * c_ang_s / lam_ang**2 / 1e-20
flam_all = flux_all * conv
err20_all = err_all * conv

valid = np.isfinite(wave_all) & np.isfinite(flam_all)
wave = wave_all[valid]
flam20 = flam_all[valid]
err20 = err20_all[valid]
...
# Dark widget styling so controls remain readable in Colab.
display(HTML("""
<style>
.jwst-dark-panel {background:#000; padding:10px; border:1px solid #444; border-radius:8px;}
.jwst-dark-panel label, .jwst-dark-panel .widget-label {color:#fff !important; font-weight:600;}
.jwst-dark-panel select, .jwst-dark-panel input {background:#111 !important; color:#fff !important; border:1px solid #666 !important;}
.jwst-dark-panel button {background:#202020 !important; color:#fff !important; border:1px solid #888 !important;}
</style>
"""))

window_dd = widgets.Dropdown(options=list(WINDOWS), value="Full paper region 1.0–3.0 μm", description="Window:", layout=widgets.Layout(width="540px"), style={"description_width":"90px"})
ymin_box = widgets.FloatText(value=-0.10, description="Y min:", layout=widgets.Layout(width="190px"), style={"description_width":"60px"})
ymax_box = widgets.FloatText(value=0.50, description="Y max:", layout=widgets.Layout(width="190px"), style={"description_width":"60px"})
save_btn = widgets.Button(description="Save current PNG", layout=widgets.Layout(width="180px"))
controls = widgets.HBox([window_dd, ymin_box, ymax_box, save_btn], layout=widgets.Layout(display="flex", flex_flow="row wrap", gap="8px", width="100%"))
controls.add_class("jwst-dark-panel")
...
plt.rcParams.update({
    ...
    "xtick.color": "white", "ytick.color": "white", "font.size": 12,
})
...
def draw(*_):
    ...
    with out:
        ...
        if np.any(cols):
            sub = sn2d[:, cols]
            ax2.imshow(sub, origin="lower", aspect="auto", extent=[x0, x1, -0.5, sub.shape[0]-0.5],
                       cmap="RdBu_r", vmin=-lim, vmax=lim, interpolation="nearest")
        ax2.set_ylabel("2-D signal-to-noise\n[dimensionless]", color="white", fontsize=12, labelpad=12)
        ax2.set_yticks([])
        ax2.tick_params(axis="x", labelbottom=False)

        if np.any(m):
            ax1.errorbar(wave[m], flam20[m], yerr=err20[m], fmt="none", ecolor="#9dd9ee",
                         elinewidth=1.2, alpha=0.72, capsize=0, zorder=1)
            ax1.step(wave[m], flam20[m], where="mid", color="#66e0d1", linewidth=1.9, zorder=2)
        for label, xpos in LINES.items():
            if x0 <= xpos <= x1:
                ax1.axvline(xpos, color="#999999", linestyle="--", linewidth=0.9, zorder=0)
                ax1.text(xpos, ymax_box.value * 0.965, label, rotation=90, ha="right", va="top",
                         fontsize=10, color="white")

        ax1.axhline(0, color="#777777", linewidth=0.8)
        ax1.set_xlim(x0, x1)
        ax1.set_ylim(ymin_box.value, ymax_box.value)
        ax1.set_xlabel(r"Observed wavelength, $\lambda_{obs}$ [$\mu$m]", color="white", fontsize=15, labelpad=12)
        ax1.set_ylabel(r"Spectral flux density, $f_\lambda$ [$10^{-20}$ erg s$^{-1}$ cm$^{-2}$ $\AA^{-1}$]",
                       color="white", fontsize=14, labelpad=14)
        ax1.grid(color="#333333", linewidth=0.55, alpha=0.65)
        ax1.tick_params(labelsize=12, direction="out", length=5, width=1.0)

        npts = int(np.count_nonzero(m))
        fig.suptitle(f"MoM-z14 — DJA/msaexp PRISM/CLEAR | observed frame {x0:.2f}–{x1:.2f} μm | {npts} native samples",
                     color="white", fontsize=15, y=0.985)
        fig.tight_layout(rect=[0.03, 0.025, 0.995, 0.965])
        plt.show()
        last_fig["fig"] = fig
        last_fig["name"] = f"{VERSION}_MOM_Z14_{x0:.2f}_{x1:.2f}UM_BLACK_PAPER_STYLE.png".replace(".", "p")

def save_current(_):
    if last_fig["fig"] is not None:
        path = PNG_DIR / last_fig["name"]
        last_fig["fig"].savefig(path, dpi=500, bbox_inches="tight", facecolor="black")
        print(f"SAVED: {path}")
...
default_png = PNG_DIR / f"{VERSION}_MOM_Z14_1p0_TO_3p0UM_BLACK_PAPER_STYLE.png"
if last_fig["fig"] is not None:
    last_fig["fig"].savefig(default_png, dpi=500, bbox_inches="tight", facecolor="black")
