...
VERSION="JWST_0241"
MODEL="MODEL_E_TIGHT_MASK_WIDGET_3_INLINE_FULL_ANALYSIS"
TARGET_GHZ=279.901
RA_DEG=53.1647632
DEC_DEG=-27.7746223
C_KMS=299792.458
...
def table_html(rows):
    body="".join(
        "<tr>"
        f"<td style='padding:8px;border-bottom:1px solid #24384d'>{name}</td>"
        f"<td style='padding:8px;text-align:right;font-weight:700;border-bottom:1px solid #24384d'>{value}</td>"
        f"<td style='padding:8px;border-bottom:1px solid #24384d'>{unit}</td>"
        f"<td style='padding:8px;border-bottom:1px solid #24384d'>{note}</td>"
        "</tr>"
        for name,value,unit,note in rows
    )
    return f"""
    <div style='background:#071019;color:#eaf2ff;border:1px solid #29445f;border-radius:10px;padding:16px;font-family:Arial,sans-serif'>
      <div style='font-size:22px;font-weight:700;margin-bottom:4px'>Tight Mask Widget 3 — Scientific Results</div>
      <div style='font-size:13px;color:#a9bdd2;margin-bottom:12px'>FITS-derived Model D line-window reconstruction; no files written</div>
...
def main():
    ...
    with fits.open(CUBE,memmap=True,do_not_scale_image_data=True) as hdul:
        ...
        for j,g in enumerate(groups):
            ...
            cube[j]=np.nanmean(
                collapse(np.asarray(data[tuple(sl)],np.float32),np_spec),axis=0
            )
    ...
    ax.set_title(f"Tight Mask Widget 3 spectrum | integrated S/N={snr:.3f}")
    ...
    ax.set_ylabel("Count")
    ax.set_title(f"Null test | percentile={percentile:.1f}% | FAP={fap:.4f}")
    ax.grid(alpha=.25); ax.legend()
    null_png=fig_bytes(fig)

    rows=[
        ("Mask area",f"{nmask:d}","pixels","Binary polygon aperture"),
        ("Mask bounds X",f"{MASK_VERTICES[:,0].min():.2f} to {MASK_VERTICES[:,0].max():.2f}","pixel","Horizontal aperture extent"),
        ("Mask bounds Y",f"{MASK_VERTICES[:,1].min():.2f} to {MASK_VERTICES[:,1].max():.2f}","pixel","Vertical aperture extent"),
        ...
        ("Pixel scale",f"{pix:.4f}","arcsec/pixel","From FITS WCS"),
        ("Null controls",f"{len(controls):d}","positions","Non-wrapping shifted apertures")
    ]

    display(HTML("<h2 style='margin-bottom:4px'>Tight Mask Widget 3 — Inline FITS Analysis</h2><p>Raw image and masked image are rendered directly below for visual confirmation.</p>"))
    display(HTML("<h3>1. Raw Model D moment-0</h3>"))
    display(IPyImage(data=raw_png))
    display(HTML("<h3>2. Same moment-0 with Tight Mask 3</h3>"))
    display(IPyImage(data=masked_png))
    display(HTML("<h3>3. Extracted aperture spectrum</h3>"))
    display(IPyImage(data=spectrum_png))
    display(HTML("<h3>4. Null-position control distribution</h3>"))
    display(IPyImage(data=null_png))
    display(HTML(table_html(rows)))

    toggle_image=widgets.Image(value=raw_png,format="png")
    toggle=widgets.ToggleButtons(
        options=[("Raw image","raw"),("Mask 3 overlay","mask")],
        value="raw",description="Compare:"
    )
    def switch(change):
        toggle_image.value=raw_png if change["new"]=="raw" else masked_png
    toggle.observe(switch,names="value")
    display(HTML("<h3>5. Interactive raw ↔ mask comparison</h3>"))
    display(widgets.VBox([toggle,toggle_image]))
