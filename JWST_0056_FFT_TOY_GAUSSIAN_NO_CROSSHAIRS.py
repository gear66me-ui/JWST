#!/usr/bin/env python3
"""
JWST_0056_FFT_TOY_GAUSSIAN_NO_CROSSHAIRS.py

Two-row toy FFT learning dashboard:
1) pure Gaussian lamp,
2) high-pass Gaussian.

Column 3 contains the raw displayed FFT power only. No horizontal line,
vertical line, marker, crosshair, halo, or glow is overplotted.
"""
...
def build_cases() -> list[dict]:
    ...
    return [
        ...
        },
        {
            ...
            "note": "Broad structure removed: low-frequency power is suppressed and can form a ring.",
        },
    ]
...
def radial_profile(power: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    radial_index = np.floor(np.hypot(xx - cx, yy - cy)).astype(int)
    radius = np.arange(radial_index.max() + 1, dtype=float)
    values = np.full(radius.shape, np.nan)
    for index in range(len(radius)):
        mask = radial_index == index
        if np.any(mask):
...
def make_dashboard(results: list[dict]) -> Path:
    ...
    for row, record in enumerate(results):
        ...
        ax.set_title("2-D FFT power — no overlays", fontsize=11)
    ...
    fig.suptitle(
        "Toy Gaussian FFT demo — no-crosshair revision\n"
        "Column 3 displays FFT power only, with no lines, markers, halos, or glow",
        fontsize=18,
    )
    output = PNG_DIR / f"{VERSION}_TOY_GAUSSIAN_NO_CROSSHAIRS_DASHBOARD.png"
...
def make_radial_plot(results: list[dict]) -> Path:
    ...
    ax.grid(alpha=0.20)
    ax.legend()
    output = PNG_DIR / f"{VERSION}_TOY_GAUSSIAN_RADIAL.png"
    fig.savefig(output, dpi=220, bbox_inches="tight", facecolor=fig.get_facecolor())
    plt.show()
    plt.close(fig)
    return output
...
def make_summary(results: list[dict]) -> tuple[Path, Path]:
    ...
    ax.set_title("Toy Gaussian FFT summary", fontsize=15, pad=16)
    png_path = PNG_DIR / f"{VERSION}_TOY_GAUSSIAN_TABLE.png"
...
def main() -> None:
    ...
    print("Change       Removed every overlay from Column 3")
    print("Column 3     Raw displayed FFT power only")
