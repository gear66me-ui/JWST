...
def main():
    ...
    with fits.open(CUBE,memmap=True,do_not_scale_image_data=True) as hdul:
        ...
        w=WCS(h).celestial
        tx,ty=w.world_to_pixel_values(RA_DEG,DEC_DEG)
    ...
    if nlab:
        ...
        for q in range(1,nlab+1):
            ...
            d=np.min(rr_arc[m]); score=np.sum(sn0[m])/(1+d)
    ...
    nbeam=max(mask.sum()/(beam_pix if np.isfinite(beam_pix) and beam_pix>0 else mask.sum()),1.0)
    ...
    try:
        ...
        popt,pcov=curve_fit(gauss,f,s,p0=p0,sigma=e,absolute_sigma=True,maxfev=20000)
