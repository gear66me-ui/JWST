...
VERSION="JWST_0243"
MODEL="MODEL_E_SELECTIVE_MASK_0242_STYLE_WIDGET"
TARGET_GHZ=279.901
RA_DEG=53.1647632
DEC_DEG=-27.7746223
C_KMS=299792.458
...
# Selective mask traced from the user's latest 1536 x 1536 annotated image.
# Display coordinates were converted back to the 128 x 128 FITS crop with Y inversion.
...
def scientific_table(rows):
    ...
    <div style='background:#071019;color:#eaf2ff;border:1px solid #29445f;border-radius:10px;padding:14px;font-family:Arial,sans-serif'>
      <div style='font-size:21px;font-weight:700;margin-bottom:10px'>JWST_0243 — Selective Mask Scientific Summary</div>
...
def main():
    ...
    nmask=int(mask.sum())
    if nmask==0:
        raise RuntimeError("Selective mask produced an empty aperture")
    ...
    masked_path=OUT_PNG/f"{VERSION}_MODEL_D_MOMENT0_SELECTIVE_MASK.png"
    raw_image.save(raw_path,format="PNG",compress_level=0,optimize=False)
    masked_image.save(masked_path,format="PNG",compress_level=0,optimize=False)

    if raw_path.stat().st_size<MIN_BYTES:
        raise RuntimeError(f"Raw master below 2.5 MB: {raw_path.stat().st_size:,} bytes")
    if masked_path.stat().st_size<MIN_BYTES:
        raise RuntimeError(f"Masked master below 2.5 MB: {masked_path.stat().st_size:,} bytes")

    drive_raw=DRIVE_PNG/raw_path.name
    drive_masked=DRIVE_PNG/masked_path.name
    drive_raw.write_bytes(raw_path.read_bytes())
    drive_masked.write_bytes(masked_path.read_bytes())
    ...
    summary=pd.DataFrame([{
        ...
        "masked_png_bytes":masked_path.stat().st_size
    }])
    csv_path=OUT_CSV/f"{VERSION}_SELECTIVE_MASK_SUMMARY.csv"
    ...
    ax.set_ylabel("Selective-aperture amplitude")
    ax.set_title(f"JWST_0243 selective-mask spectrum | integrated S/N={snr:.3f}")
    ...
    rows=[
        ("Mask area",f"{nmask:d}","pixels","Binary selective polygon"),
        ...
        ("Integrated line statistic",f"{target_flux:.7f}","aperture units","Sum across selected grouped channels"),
        ...
        ("Raw master size",f"{raw_path.stat().st_size/1_000_000:.3f}","MB","1024 x 1024, PNG compression disabled"),
        ("Masked master size",f"{masked_path.stat().st_size/1_000_000:.3f}","MB","1024 x 1024, PNG compression disabled")
    ]
    ...
    download_raw=widgets.Button(description="Download raw PNG",button_style="success")
    download_mask=widgets.Button(description="Download masked PNG",button_style="warning")
    download_csv=widgets.Button(description="Download summary CSV",button_style="info")
    status=widgets.HTML()
    ...
    report=widgets.HTML(scientific_table(rows))
    paths=widgets.HTML(
        ...
        f"<b>Google Drive masked:</b> {drive_masked}<br>"
        f"<b>Google Drive summary:</b> {drive_csv}</div>"
    )

    display(widgets.HTML("<h3>JWST_0243 — Model D master + selective mask</h3>"))
    display(toggle)
    display(image)
    display(widgets.HBox([download_raw,download_mask,download_csv]))
    display(status)
    display(widgets.Image(value=spectrum_png,format="png"))
    display(widgets.Image(value=null_png,format="png"))
    display(report)
    display(paths)
    ...
    print(f"MASKED MASTER: {drive_masked}")
