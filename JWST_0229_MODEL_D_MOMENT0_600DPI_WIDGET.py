...
VERSION="JWST_0229"
MODEL="MODEL_D_MOMENT0_600DPI_WIDGET"
TARGET_GHZ=279.901
RA_DEG=53.1647632
DEC_DEG=-27.7746223
C_KMS=299792.458
...
def main():
    ...
    p_png=OUT_PNG/f"{VERSION}_MODEL_D_MOMENT0_600DPI_UNTOUCHED.png"
    ...
    fig.savefig(p_png,dpi=600,bbox_inches="tight",pad_inches=0.05)
    plt.close(fig)
    ...
    scsv=OUT_CSV/f"{VERSION}_MODEL_D_MOMENT0_600DPI_SUMMARY.csv"
    summary.to_csv(scsv,index=False)
    (DRIVE_CSV/scsv.name).write_bytes(scsv.read_bytes())

    raw=p_png.read_bytes()
    image_widget=widgets.Image(value=raw,format="png")
    display(widgets.VBox([
        widgets.HTML(
            f"<h3>Model D moment-0 baseline — untouched 600 DPI PNG</h3>"
            f"<b>{p_png.stat().st_size/1_000_000:.3f} MB</b>"
        ),
        image_widget
    ]))
