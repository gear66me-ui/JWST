...
def find_spectral_axis(header):
    for ax in range(1, int(header['NAXIS'])+1):
        if 'FREQ' in str(header.get(f'CTYPE{ax}','')).upper(): return ax
...
def bin_indices(freq):
    lo = TARGET_GHZ - 0.45
    hi = TARGET_GHZ + 0.45
    edges = np.arange(lo, hi + BIN_MHZ/1000, BIN_MHZ/1000)
    centers = 0.5*(edges[:-1]+edges[1:])
    groups=[]
    for a,b in zip(edges[:-1], edges[1:]):
        idx=np.where((freq>=a)&(freq<b))[0]
        groups.append(idx)
...
def main():
    ...
    with fits.open(CUBE, memmap=True, do_not_scale_image_data=True) as hdul:
        ...
        centers=centers[valid]; groups=[groups[i] for i in valid]
        nb=len(groups); ny=ys.stop-ys.start; nx=xs.stop-xs.start
        ...
        for j,g in enumerate(groups):
            ...
            cube5[j]=np.nanmean(arr,axis=tuple(range(1,arr.ndim-2)) if arr.ndim>3 else 0)
        del data; gc.collect()

    yy,xx=np.indices((ny,nx)); rr=np.hypot(xx-nx/2,yy-ny/2)
    offmask=rr>20
    rms_chan=np.array([robust_rms(im[offmask]) for im in cube5])
    line_sel=np.abs(centers-TARGET_GHZ)<=LINE_HALF_WIDTH_GHZ
    if line_sel.sum()<3: raise RuntimeError('Insufficient line bins')
    ...
    for it in range(3):
        ...
        if nlab:
            ...
            for k in range(1,nlab+1):
                m=labs==k
                if m.any(): cands.append((np.sum(sn[m]),m))
            mask=max(cands,key=lambda t:t[0])[1]
        if not mask.any(): mask=rr<=4
        spec=np.array([np.nansum(im[mask]) for im in cube5])
        line_spec=spec[line_sel]
        weights=line_spec.copy()
        if np.allclose(weights,0): weights=np.ones_like(weights)
        diagnostics.append((it+1,int(mask.sum()),float(np.nanmax(sn))))

    spec=np.array([np.nansum(im[mask]) for im in cube5])
    nbeam=max(mask.sum(),1)
    err=rms_chan*np.sqrt(nbeam)
    fline=centers[line_sel]; sline=spec[line_sel]; eline=err[line_sel]
    denom=np.sum(sline)
    ...
    df=pd.DataFrame({'frequency_GHz':centers,'flux_sum_mJy_beam':spec*1000,'sigma_mJy_beam':err*1000,'is_line_window':line_sel})
    csv=OUT_CSV/f'{VERSION}_PAPER_METHOD_SPECTRUM.csv'; df.to_csv(csv,index=False)
    df.to_csv(DRIVE_CSV/csv.name,index=False)
    mask_csv=OUT_CSV/f'{VERSION}_EXTRACTION_MASK_PIXELS.csv'
    my,mx=np.where(mask)
    pd.DataFrame({'y_cutout':my,'x_cutout':mx,'y_full':my+ys.start,'x_full':mx+xs.start}).to_csv(mask_csv,index=False)
    pd.read_csv(mask_csv).to_csv(DRIVE_CSV/mask_csv.name,index=False)
    ...
    if np.isfinite(centroid): plt.axvline(centroid,ls=':',lw=1.4,label=f'Centroid {centroid:.6f} GHz')
    plt.axvspan(TARGET_GHZ-LINE_HALF_WIDTH_GHZ,TARGET_GHZ+LINE_HALF_WIDTH_GHZ,alpha=.08)
    plt.xlabel('Observed frequency (GHz)'); plt.ylabel('Integrated aperture flux (mJy beam⁻¹)')
    plt.title('JADES-GS-z11-0 — paper-style iterative ALMA extraction')
    plt.grid(alpha=.2); plt.legend(); plt.tight_layout()
    png=OUT_PNG/f'{VERSION}_PAPER_METHOD_SPECTRUM.png'; plt.savefig(png,dpi=180); plt.close()
    ...
    plt.xlabel('Cutout x pixel'); plt.ylabel('Cutout y pixel'); plt.tight_layout()
    mpng=OUT_PNG/f'{VERSION}_LINE_MAP_MASK.png'; plt.savefig(mpng,dpi=180); plt.close()
    ...
    for it,npix,peak in diagnostics: print(f'Iteration {it}: mask pixels={npix:4d} | peak S/N map={peak:7.3f}')
