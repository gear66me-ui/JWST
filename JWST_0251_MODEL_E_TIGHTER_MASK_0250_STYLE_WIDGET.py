...
VERSION="JWST_0251"
MODEL="MODEL_E_TIGHTER_MASK_0250_STYLE_ANALYSIS"
TARGET_GHZ=279.901
RA_DEG=53.1647632
DEC_DEG=-27.7746223
C_KMS=299792.458
...
# Tight contour traced directly from the user's latest 1536x1536 annotated image.
# Coordinates below are already in the 128x128 extraction frame used by JWST_0250.
MASK_VERTICES=np.array([
    [63.00,58.75],[63.50,60.17],[64.33,60.58],[67.50,60.58],
    [68.42,60.25],[69.08,59.67],[68.92,58.17],[68.33,57.67],
    [67.17,57.25],[65.92,57.25],[64.08,57.67],[63.42,58.00]
],dtype=float)
...
def shifted_nonwrapping(template,dx,dy):
    ...
    if y1>y0 and x1>x0:
        out[y0:y1,x0:x1]=template[sy0:sy0+y1-y0,sx0:sx0+x1-x0]
    return out
...
def rgb_master(moment,vertices=None):
    ...
    if vertices is not None:
        draw=ImageDraw.Draw(image); s=OUTPUT_PIXELS/128.0
        pts=[(float(x)*s,float(y)*s) for x,y in vertices]; pts.append(pts[0])
        draw.line(pts,fill=(255,35,20),width=5,joint="curve")
    return image
...
def main():
    ...
    nmask=int(mask.sum())
    if nmask==0: raise RuntimeError("Tighter mask is empty")
    ...
    raw_image=rgb_master(moment); masked_image=rgb_master(moment,MASK_VERTICES)
    raw_path=OUT_PNG/f"{VERSION}_MODEL_D_MOMENT0_RAW.png"
    mask_path=OUT_PNG/f"{VERSION}_MODEL_E_TIGHTER_MASK.png"
    ...
    ax.set_title(f"Tighter-mask spectrum | integrated S/N={snr:.3f}"); ax.grid(alpha=.25); ax.legend()
    ...
    rows=[
        ("Mask area",f"{nmask}","pixels","Tighter user-drawn polygon"),
        ("Mask X bounds",f"{MASK_VERTICES[:,0].min():.3f} to {MASK_VERTICES[:,0].max():.3f}","pixels","Direct JWST_0250 display/extraction frame"),
        ("Mask Y bounds",f"{MASK_VERTICES[:,1].min():.3f} to {MASK_VERTICES[:,1].max():.3f}","pixels","Direct JWST_0250 display/extraction frame"),
    ...
    toggle=widgets.ToggleButtons(options=[("Raw Model D","raw"),("Tighter mask","mask")],value="raw",description="Compare:")
    ...
    display(widgets.HTML(f"<h3>{VERSION} — tighter-mask analysis</h3>"))
    ...
    display(widgets.HTML("<h4>Tighter mask overlay</h4>")); display(widgets.Image(value=mask_bytes,format="png",layout=widgets.Layout(width="900px",max_width="100%")))
